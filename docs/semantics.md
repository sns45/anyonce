...
- If `run` throws (TypeScript) or returns an error (Go), the engine calls `abandon` with the claim's fence and rethrows, so the key is immediately free for a retry. In Go the error is wrapped (`anyonce: handler failed: %w`). `anyonce.Execute` itself has no `recover`: a handler panic is recovered by the doors (`go/internal/httpx` for `httpmw` and `webhookmw`, and `anyqmw`), which turn it into an error from `run` so the engine abandons the claim, and then re-raise the panic.
...
`withProtocolHeaders(res, headers)` is exported for a door that renders its own problems before the bridge runs, so an `onError` override still gets `Cache-Control: no-store` and headers such as `Retry-After`. In Go the shared door helpers (`KeyRejected`, `ReadBody`, `Execute` on `httpx.Door`) live in the module's `go/internal/httpx` package, so a new Go door is added inside this module, alongside `httpmw` and `webhookmw`. A non HTTP door (a queue, a job runner) calls `execute` or `anyonce.Execute` directly, as `@anyonce/anyq` and `anyqmw` do. A Go door that calls `anyonce.Execute` directly must recover panics around its handler inside `run` and return them as an error, as `anyqmw` does; otherwise a panicking handler skips `abandon` and the record stays `in_flight` until the lease expires.
