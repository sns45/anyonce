...
`conformance/vectors/` is the source of truth, and each runner ships its own copy so it works outside this repository. The `@anyonce/conformance` build copies the directory into `packages/conformance/vectors/` (gitignored), the tarball ships it, and `loadVectors()` with no argument reads that copy, falling back to the repository's `conformance/vectors/` before a build. The Go module keeps a committed copy in `go/conformance/vectors/` and embeds it: `conformance.DefaultVectors()` reads the embedded copy, `conformance.Run` uses it unless `Options.VectorsDir` names a directory, and `conformance.LoadVectors(dir)` reads any directory. A parity test fails when `go/conformance/vectors/` and `conformance/vectors/` differ by a byte.
...
4. Copy the file to the same path under `go/conformance/vectors/` (the Go module embeds that copy), then validate: `bun run vectors:validate`, `bun run build`, then `bun test packages/conformance` and the Go runner's tests (`GOROOT= go test -C go ./conformance/...`, whose parity test fails until the Go copy matches). Neither runner needs a code change for a new vector.
