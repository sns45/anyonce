...
| dynamodb | TS, Go | strongly consistent reads (ConsistentRead) | one conditional UpdateItem; refusal returns the old item | yes, ttl attribute (seconds, expires_at plus 60 s grace) | ensureTable(client) in TS, EnsureTable(ctx, client, table) in Go, both for tests and local development while production tables come from infrastructure code, or create pk (S) only with TTL on ttl | pk (S) only, the scope and key joined, so every claim is a point write on its own partition (Q22); one write per request, one more per replay read; items capped at 400 KB so maxResultBytes must be at most 300 KiB (Q20) | 300 KiB |
...
Known CI flake (d1 row): the D1 workers suite (`packages/stores/workers/d1.test.ts`, run through `bun run test:workers`) failed once in CI with `Network connection lost`, most likely the vitest-pool-workers per-file storage frame pop racing an in-flight D1 statement (see the comment in `test/workers/vitest.config.ts`). The mitigation already in place is `singleWorker: true`, which runs the D1 and Durable Objects workers files one at a time instead of in parallel; it lowers the load that made the race show rather than removing the race itself. Ten consecutive local runs of `bun run test:workers` did not reproduce it. The only permitted CI action for this failure is `gh run rerun <id> --failed` for the affected job; never a retry loop, a sleep or a test change made without first finding and removing a real cause.
