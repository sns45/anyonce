...
describe('package hygiene', () => {
  test('REQ-ST-KV-1: the package declares five store subpaths and the migrations files, no dependencies, and optional peers only', () => {
    ...
    expect(pkg.exports['./migrations/*']).toBe('./migrations/*');
    expect(Object.keys(pkg.exports)).not.toContain('./kv');
    for (const [subpath, entry] of Object.entries(pkg.exports)) {
      if (subpath === './migrations/*') continue;
  ...
  test('REQ-ST-D1-1: the migrations subpath resolves the committed D1 and Postgres migration files', () => {
    for (const file of ['migrations/d1/0001_anyonce.sql', 'migrations/postgres/0001_anyonce.sql']) {
      expect(readFileSync(join(pkgDir, file), 'utf8').length).toBeGreaterThan(0);
    }
  });
