...
- The Redis Lua scripts and the Postgres and SQLite schemas exist once per language and must stay byte equal across them: `packages/stores/src/redis.ts` against `go/store/redis/scripts.go`, `packages/stores/src/sql.ts` against `go/store/postgres/schema.sql` and `go/store/sqlite/schema.sql`, and the committed migrations under `packages/stores/migrations`. `packages/stores/test/parity.test.ts` enforces it, so change both sides in the same commit.
