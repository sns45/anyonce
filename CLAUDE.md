...
- `bun run test:examples` (the examples' smoke tests; needs DynamoDB Local and Postgres from the compose stack below), `bun run test:examples:workers` (the Worker example under vitest-pool-workers), `bun run bench` (NFR-1 HTTP adapter overhead), `bun run release:dry-run` (the 0.1.0 dry run over a `git archive` of HEAD, so commit first; it never publishes, tags or pushes).
...
- `bun run test:services` needs that compose stack up: it runs the connectivity check plus the store suites under `packages/stores/services` and the queue suites under `packages/anyq/services`. `bun run test:workers` runs the D1 and Durable Objects store suites alongside the P2 runtime matrix. The Go store and `anyqmw` tests read `ANYONCE_REQUIRE_SERVICES`: unset they skip a missing service, set to `1` (as CI does) they fail instead.
